...
func AppHostCount(instance *cyndi.CyndiPipeline, value int64) {
	hostCount.WithLabelValues(instance.Spec.AppName, instance.Status.PipelineVersion).Set(float64(value))
}

func ValidationFinished(instance *cyndi.CyndiPipeline, threshold int64, ratio float64, isValid bool) {
	inconsistencyThreshold.WithLabelValues(instance.Spec.AppName, instance.Status.PipelineVersion).Set(float64(threshold) / 100)
	inconsistencyRatio.WithLabelValues(instance.Spec.AppName, instance.Status.PipelineVersion).Set(ratio)

	if !isValid {
		validationFailedCount.WithLabelValues(instance.Spec.AppName, instance.Status.PipelineVersion).Inc()
	}
}
