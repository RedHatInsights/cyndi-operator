	...
	Context("With database connection", func() {
		...
		Describe("Counting hosts", func() {
			...
			It("Counts insights hosts with additionalFilters", func() {
				...
				filters := []map[string]string{{"name": "testFilter", "type": "com.redhat.insights.kafka.connect.transforms.Filter", "if": "!!record.headers().lastWithName('insights_id').value()", "where": "canonical_facts ? 'insights_id'"}}
